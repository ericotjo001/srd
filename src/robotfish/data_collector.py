...
class FishUnitDataCollector():
    ...
    def display_srd_data(self, dargs, save_dir=None):
        ...
        iters = average_every_n(self.iter, n=dargs['average_every']).astype(int)
        plt.gca().plot(iters, average_every_n(self.energy, n=dargs['average_every']), label='avg energy')
        plt.gca().set_ylim([0,1])
